...
O projeto foi arquitetado sob os mais rígidos padrões de engenharia de testes, garantindo manutenibilidade, isolamento de escopo e escalabilidade:

* **Linguagem:** JavaScript (ES6+)
* **Framework Principal:** WebdriverIO (WDIO v8)
* **Driver Mobile:** Appium (UiAutomator2 para Android)
* **Gerenciador de Testes:** Mocha (BDD - `describe`, `it`)
* **Asserções:** Expect (nativo do WDIO)
* **Padrão de Projeto:** Page Objects (Encapsulamento de elementos da interface e ações)
* **Estratégia de Dados:** Data-Driven Testing (Massa de dados parametrizada via arquivo JSON)
* **Geração de Evidências:** Allure Report (Com capturas automáticas de tela em cenários de falha)
* **CI/CD:** GitHub Actions (Configurado com emulador virtual reativo rodando em ambiente macOS)
...
```text
project-mobile-javascript/
├── .github/workflows/
│   └── android-tests.yml     # Pipeline de Integração Contínua (GitHub Actions)
├── app/
│   └── wdioNativeDemoApp.apk # APK do aplicativo sob teste (Manual)
├── config/
│   ├── wdio.android.conf.js  # Configurações de execução exclusivas do Android
│   └── wdio.ios.conf.js      # Estrutura base para futuras execuções iOS
├── test/
│   ├── data/                 # Massa de dados dinâmica (JSON)
│   │   └── usuarios.json
│   ├── pageobjects/          # Classes de representação de Telas (Page Objects)
│   │   └── login.page.js
│   └── specs/                # Cenários de testes reais (Scripts de Automação)
│       └── login.spec.js
├── package.json              # Gerenciador de dependências e scripts do Node.js
└── README.md                 # Manual de instalação, configuração e execução

## 🛠️ Pré-requisitos do Sistema
Antes de iniciar a configuração local, certifique-se de ter instalado em sua máquina:

Node.js (Versão 18 LTS ou superior)

Java JDK (Versão 11 ou 17) -> Obrigatório para o ecossistema do Android SDK

🚀 Configuração do Ambiente Android via Terminal (Modo Binário / CLI)
Caso o ecossistema do Android SDK tenha sido instalado diretamente em modo binário e local no projeto (sem a necessidade de abrir a interface pesada do Android Studio), siga o passo a passo abaixo no terminal (PowerShell no Windows) para gerenciar o emulador:

1. Criar o Aparelho Virtual (AVD)
Execute o comando abaixo para gerar um telemóvel virtual chamado Android_Carrefour utilizando a imagem de sistema descompactada do Android 30:

PowerShell
avdmanager create avd -n Android_Carrefour -k "system-images;android-30;google_apis;x86_64"
Nota: Se o terminal perguntar "Do you wish to create a custom hardware profile? [no]", pressione Enter para aceitar o padrão.

2. Listar os Emuladores Disponíveis
Para validar se o aparelho foi registrado corretamente pelo binário do sistema, execute:

PowerShell
& "$env:USERPROFILE\AppData\Local\Android\Sdk\emulator\emulator.exe" -list-avds
O terminal deverá retornar o nome: Android_Carrefour.
...
& "$env:USERPROFILE\AppData\Local\Android\Sdk\emulator\emulator.exe" -avd nightwatch-android-11

Reiniciar dispositivo:
& "$env:USERPROFILE\AppData\Local\Android\Sdk\emulator\emulator.exe" -avd Android_Carrefour -no-snapshot-load

⚠️ Importante: Este comando irá bloquear esta aba do terminal enquanto o telemóvel estiver ligado. Aguarde de 1 a 2 minutos até que o sistema do Android carregue completamente na janela flutuante e mantenha a janela aberta.

📥 Inicialização do Projeto
Com o emulador móvel devidamente carregado em seu monitor, abra uma nova aba de terminal no seu VS Code, navegue até a raiz do projeto e execute:

Bash
# Instala os pacotes do WebdriverIO, Appium Service e reporters listados no package.json
npm install
⚠️ Nota sobre o Aplicativo (APK)
Certifique-se de que a pasta app/ existe na raiz do seu projeto e que o aplicativo sob teste está localizado e nomeado exatamente como ./app/wdioNativeDemoApp.apk.

Caso precise fazer o download manual do binário, utilize a aba oficial de distribuições do WebdriverIO no GitHub (Acesse as Releases do native-demo-app), baixe o arquivo .apk e coloque-o na pasta correta.

🧪 Execução Local dos Testes Funcionais
Para rodar a suíte de testes contra o emulador CLI que você acabou de ligar, execute o seguinte atalho mapeado no arquivo package.json:

Bash
# Executa a automação mobile direcionada ao arquivo de configuração do Android
...
Para compilar os dados brutos e abrir o painel de relatórios interativo em seu navegador padrão, execute:

Bash
# 1. Instala globalmente a CLI do Allure (caso não possua em sua máquina)
npm install -g allure-commandline

# 2. Processa os resultados de 'allure-results' e abre o servidor web local do relatório
allure serve allure-results
🚀 Integração Contínua e Entrega Contínua (CI/CD)
O pipeline de automação está configurado de forma resiliente usando o GitHub Actions através do arquivo .github/workflows/android-tests.yml.

Toda vez que uma alteração é enviada via git push ou um pull_request é aberto para as branches principais (main ou master), a esteira de CI executa os seguintes passos:

Realiza o provisionamento de uma máquina virtual macOS dedicada (essencial para suporte de emulação mobile em nuvem).

Prepara o ambiente instalando as versões estáveis do Node.js e Java SDK.

Cria, inicializa e valida um emulador Android reativo em tempo de execução via script.

Dispara as rotinas de teste mobile funcionais utilizando o Appium.

Gera o relatório analítico do Allure e exporta os logs juntamente com os arquivos de imagens gerados como artefatos zipados anexados à execução da pipeline.


O arquivo está pronto, muito bem detalhado tanto para o uso de interface visual quanto para o uso avançado via linha de comando (CLI) que você executou!
...
> [!IMPORTANT]
> This project uses the WebdriverIO native demo app which can be found [here](https://github.com/webdriverio/native-demo-app).
...
- The appium server and needed drivers (already included in this Project)
- The `appium-inspector-plugin` package (already included in this Project)
...
When using the Appium Inspector, you'll need to configure capabilities that match your target device and app. Here are examples based on this project configuration:
...
The Login screen holds a simple implementation of a Login and SignUp form. This project holds 2 different test files for the Login screen.
...
There are 2 types of navigation tests explained in this project.
...
The app has a WebView that will automatically load the WebdriverIO documentation page. This project holds 2 test files:
...
This project provides a setup for testing with BrowserStack. Please check the [BrowserStack](./config/browserstack) folder to see the setup for iOS and Android.
...
This project provides a setup for testing with TestingBot. Please check the [TestingBot](./config/testingbot)-folder to see the setup for Android.
...
This project provides a setup for testing with LambdaTest.
